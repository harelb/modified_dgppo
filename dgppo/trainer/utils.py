...
#         start_graph: Optional[GraphsTuple] = None # <-- ADDED PARAMETER
# ) -> Rollout:
    
#     # Use the provided start_graph if available, otherwise call env.reset()
#     if start_graph is not None:
#         init_graph = start_graph
#     else:
#         init_graph = env.reset(key, transition_index=transition_index)
#         # We need to split the key here so that subsequent calls to actor don't use the same key
#         key, _ = jax.random.split(key)

#     def body_(data, key_):
#         graph, rnn_state = data
#         # REVISED: The actor is now always called with a key, which matches the new act_fn signature.
...
#     jdebug.print("next_graphs.nodes shape: {}", next_graphs.nodes.shape)
#     jdebug.print("next_graphs.n_node shape: {}", next_graphs.n_node.shape)
