...
import numpy as np
import pickle # Still needed for ah_lagr if not using np.save/load directly
import flax.linen as nn
import flax.serialization # <--- NEW IMPORT
...
class InforMARLLagr(InforMARL):
    ...
    def __init__(
        ...
        # --- IMPORTANT: Assuming policy_train_state and Vl_train_state are initialized in InforMARL base class ---
        # They should also be instances of flax.training.train_state.TrainState
        # For example, in InforMARL's __init__ you might have:
    ...
    def save(self, save_dir: str, step: int):
        ...
        # Save policy TrainState components
        policy_params_bytes = flax.serialization.to_bytes(self.policy_train_state.params)
        policy_opt_state_bytes = flax.serialization.to_bytes(self.policy_train_state.opt_state)
        with open(os.path.join(model_dir, 'policy_params.msgpack'), 'wb') as f:
            f.write(policy_params_bytes)
        with open(os.path.join(model_dir, 'policy_opt_state.msgpack'), 'wb') as f:
            f.write(policy_opt_state_bytes)

        # Save Vl TrainState components
        Vl_params_bytes = flax.serialization.to_bytes(self.Vl_train_state.params)
        Vl_opt_state_bytes = flax.serialization.to_bytes(self.Vl_train_state.opt_state)
        with open(os.path.join(model_dir, 'Vl_params.msgpack'), 'wb') as f:
            f.write(Vl_params_bytes)
        with open(os.path.join(model_dir, 'Vl_opt_state.msgpack'), 'wb') as f:
            f.write(Vl_opt_state_bytes)

        # Save Vh TrainState components
        Vh_params_bytes = flax.serialization.to_bytes(self.Vh_train_state.params)
        Vh_opt_state_bytes = flax.serialization.to_bytes(self.Vh_train_state.opt_state)
        with open(os.path.join(model_dir, 'Vh_params.msgpack'), 'wb') as f:
            f.write(Vh_params_bytes)
        with open(os.path.join(model_dir, 'Vh_opt_state.msgpack'), 'wb') as f:
            f.write(Vh_opt_state_bytes)

        # Save the ah_lagr array
        np.save(os.path.join(model_dir, 'ah_lagr.npy'), self.ah_lagr)

        print(f"Algorithm state saved to {model_dir}")


    def load(self, load_dir: str, step: int):
        path = os.path.join(load_dir, str(step))

        # Load policy TrainState components and recreate
        with open(os.path.join(path, 'policy_params.msgpack'), 'rb') as f:
            policy_params = flax.serialization.from_bytes(self.policy_train_state.params, f.read())
        with open(os.path.join(path, 'policy_opt_state.msgpack'), 'rb') as f:
            policy_opt_state = flax.serialization.from_bytes(self.policy_train_state.opt_state, f.read())
        self.policy_train_state = self.policy_train_state.replace(params=policy_params, opt_state=policy_opt_state)

        # Load Vl TrainState components and recreate
        with open(os.path.join(path, 'Vl_params.msgpack'), 'rb') as f:
            Vl_params = flax.serialization.from_bytes(self.Vl_train_state.params, f.read())
        with open(os.path.join(path, 'Vl_opt_state.msgpack'), 'rb') as f:
            Vl_opt_state = flax.serialization.from_bytes(self.Vl_train_state.opt_state, f.read())
        self.Vl_train_state = self.Vl_train_state.replace(params=Vl_params, opt_state=Vl_opt_state)

        # Load Vh TrainState components and recreate
        with open(os.path.join(path, 'Vh_params.msgpack'), 'rb') as f:
            Vh_params = flax.serialization.from_bytes(self.Vh_train_state.params, f.read())
        with open(os.path.join(path, 'Vh_opt_state.msgpack'), 'rb') as f:
            Vh_opt_state = flax.serialization.from_bytes(self.Vh_train_state.opt_state, f.read())
        self.Vh_train_state = self.Vh_train_state.replace(params=Vh_params, opt_state=Vh_opt_state)

        # Load the ah_lagr array
        self.ah_lagr = jnp.array(np.load(os.path.join(path, 'ah_lagr.npy')))

        print(f"Algorithm state loaded from {path}")
